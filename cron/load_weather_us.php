...
*/
function isInOrder($wsid, $first_ts, $tablename) {
	global $db;
	...
	*/
	if ($tablename == 'wx_singlesensor1m') {
		...
		$wc = "WHERE wsid={$wsid} AND timestamp < '{$first_ts}' ";
	}
...
function calcCumMinuteRF_ss($wsid, $first_ts){
	...
	$inOrder = TRUE;
	$last_csv_ts = isInOrder($wsid, $first_ts,'wx_singlesensor1m');
	$inOrder = $last_csv_ts === FALSE ? FALSE : TRUE;
	...
	if ( !$inOrder) {
		...
		$last_csv_rf = $db->getScalar("SELECT aggval FROM  wx_singlesensor1m WHERE wsid={$wsid} AND sensorcol='rf' AND timestamp='{$last_csv_ts}'");
		$sql = "SELECT MIN(timestamp) AS firstrf FROM  wx_singlesensor1m WHERE wsid = '{$wsid}' AND sensorcol'rf' "
		...
		if (empty($first_rf_ts)) {
			...
			$wsql = "SELECT timestamp, dataval, agg_val FROM wx_singlesensor1m WHERE wsid = '{$wsid}' AND sensorcol='rf' "
				  ."AND timestamp >= '{$first_rf_ts}' ORDER BY timestamp";
			$wrows = $db->query($wsql);
			$agg_val = 0.0;
			$sqls = array();
			foreach ($wrows as $wrow) {
...
function calcCumMinuteRF($idsite, $wsid, $first_ts, $tablename){
	...
	$dt = substr($first_ts,0,10);
	$inOrder = TRUE;
	$last_csv_ts = isInOrder($wsid, $first_ts,$tablename);
	...
	if ( !$inOrder ) {
		...
		// IN-ORDER processing
		if ($debug) echo "\nProcessing {$wsid} From {$first_ts} ";
		$last_csv_rf = $db->getScalar("SELECT cumrf FROM  {$tablename} WHERE wsid={$wsid} AND timestamp='{$last_csv_ts}'");
		$sql = "SELECT MIN(timestamp) AS firstrf FROM  {$tablename} WHERE wsid = '{$wsid}' "
	...
	if ($tablename == 'minuteweatherfg'){
	    ...
	    foreach ($dates as $dtrow) {
			...
			if (empty($first_rf_ts)) {
				...
				$wsql = "SELECT timestamp, rf2, cumrf2 FROM {$tablename} WHERE idsite = {$idsite} AND wsid={$wsid} AND timestamp BETWEEN '{$first_rf_ts}' AND '{$midnight}' ORDER BY timestamp";
				$wrows = $db->query($wsql);
				$cumrf2 = 0.0;
				$sqls = array();
				foreach ($wrows as $wrow) {
