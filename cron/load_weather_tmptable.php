...
$Logger = new Logger("load_wx_tmp.log");
$debug = false;
...
******************************************************/
/****************** NOTE 2019-06-11 ******************
 * adapted from load_weather_oo.php to handle all stations
 * except for AQU (and perhaps FG under a separte cron
******************************************************/
...
*/
function isInOrder($idsite, $wsid, $first_ts, $tmp_tablename) {
	global $db;
	/*
	 ...
	 * 	return last timestamp if it IS in order, false otherwise. Note that the
	 * 	tmp_tablename is populated with data copied out from the dest weather table
	*/
	$wc = "WHERE idsite={$idsite} AND wsid={$wsid} AND timestamp < '{$first_ts}' ";
	$sql = "SELECT MAX(timestamp) FROM {$tmp_tablename} {$wc} ";
	$ts = $db->getScalar($sql);
...
function prepOutOfOrder_load($tablename, $ts, $idsite, $wsid) {
	global $db;
	// a file will have only one date per wsid
	if ($tablename == "minuteweather") {
		$tmp_tablename =  "minutewx_import";
	} elseif ($tablename == "minuteweatherfg") {
		$tmp_tablename = "minutewxfg_import";
	}
	$sql = "DELETE FROM {$tmp_tablename} WHERE idsite={$idsite} AND wsid={$wsid};";
	$db->exec($sql);
	$dtrange = substr($ts,0,10).' 00:01:00 AND '.substr($ts,0,10).' 23:59:59';
	// get what we have for the day
	$sql = "REPLACE INTO {$tmp_tablename} SELECT * FROM {$tablename} WHERE idsite={$idsite} AND wsid = {$wsid} AND {$dtrange}";
	$db->exec($sql);
}
...
function process_file($dirpath, $fname){
	...
	$sql19 = "REPLACE INTO minutewxfg_import(wsid, idsite, timestamp, recno, baro, rf, rf2, temp, RH, solarkw, solarmj, windspeed, winddir, vmc, ec, soiltemp, "
			." gflux, soiltemptop, soiltempbot, lightning, batt, local_ts)\n VALUES ";
	...
	if (($handle = fopen($fullfname, "r")) !== FALSE) {
		...
		if (empty($error)) {
			...
			if ($debug) echo "call calcCumMinuteRF with {$idsite} {$wsid} {$first_ts} {$tmp_tablename} \n";
			$notes .= calcCumMinuteRF($idsite, $wsid, $first_ts, $tmp_tablename);
			$dest_table = $wsids[$wsid]['dest_table'];
			if ($tablename == "minuteweatherfg") {
				if ($debug) echo "call AggregateToWeather with 1. {$idsite} 2. {$wsid} 3. {$first_ts} 4. {$last_ts} 5. {$tmp_tablename} 6. {$dest_table} \n";
				$notes .= AggregateToWeather2rf($idsite, $wsid, $first_ts, $last_ts, $tmp_tablename, $dest_table);
				$sql1 = "REPLACE INTO minuteweatherfg(wsid, idsite, timestamp, created, recno, baro, rf, temp, "
					."RH, solarkw, solarmj, windspeed, winddir, vmc, ec, soiltemp, gflux, soiltemptop, soiltempbot, "
					."lightning, batt, rf2, filler2, filler3, filler4, pump, valve_open, valve_close, relay_status, "
					."agg, cumrf, cumrf2, local_ts) "
				."SELECT wsid, idsite, timestamp, created, recno, baro, rf, temp, "
					."RH, solarkw, solarmj, windspeed, winddir, vmc, ec, soiltemp, gflux, soiltemptop, soiltempbot, "
					."lightning, batt, rf2, filler2, filler3, filler4, pump, valve_open, valve_close, relay_status, "
					."agg, cumrf, cumrf2, local_ts "
				."FROM minutewxfg_import tmp "
					."INNER JOIN minuteweatherfg wx ON tmp.wsid = wx.wsid AND tmp.timestamp = wx.timestamp "
					."WHERE tmp.cumrf <> wx.cumrf OR tmp.cumrf2 <> wx.cumrf; ";

				$sql2 = "REPLACE INTO minuteweatherfg(wsid, idsite, timestamp, created, recno, baro, rf, temp, "
					."RH, solarkw, solarmj, windspeed, winddir, vmc, ec, soiltemp, gflux, soiltemptop, soiltempbot, "
					."lightning, batt, rf2, filler2, filler3, filler4, pump, valve_open, valve_close, relay_status, "
					."agg, cumrf, cumrf2, local_ts) "
				."SELECT wsid, idsite, timestamp, created, recno, baro, rf, temp, "
					...
					."agg, cumrf, cumrf2, local_ts "
				."FROM minutewxfg_import tmp "
					."LEFT JOIN minuteweatherfg wx ON tmp.wsid = wx.wsid AND tmp.timestamp = wx.timestamp "
					."WHERE wx.timestamp IS NULL; ";
			} else {
				if ($debug) echo "call AggregateToWeather with 1. {$idsite} 2. {$wsid} 3. {$first_ts} 4. {$last_ts} 5. {$tmp_tablename} 6. {$dest_table} \n";
				$notes .= AggregateToWeather($idsite, $wsid, $first_ts, $last_ts, $tmp_tablename, $dest_table);
				if (isset($clones[$wsid])) {
					...
					echo "\nCSQL\n{$csql}";
					$db->exec($csql);
					$notes .= AggregateToWeather($clones[$wsid]['id_dst'], $clones[$wsid]['wsid_dst'], $first_ts, $last_ts, $oo_tablename, $dest_table);
				}
				$sql1 = "REPLACE INTO minuteweather(wsid, idsite, timestamp, created, recno, baro, rf, temp, "
					."RH, solarkw, solarmj, windspeed, winddir, vmc, ec, soiltemp, gflux, soiltemptop, soiltempbot, "
					."lightning, batt, agg, cumrf,  local_ts) "
				."SELECT wsid, idsite, timestamp, created, recno, baro, rf, temp, "
					."RH, solarkw, solarmj, windspeed, winddir, vmc, ec, soiltemp, gflux, soiltemptop, soiltempbot, "
					."lightning, batt, agg, cumrf, local_ts "
				."FROM minutewx_import tmp "
					."INNER JOIN minuteweather wx ON tmp.wsid = wx.wsid AND tmp.timestamp = wx.timestamp "
					."WHERE tmp.cumrf <> wx.cumrf ;";

				$sql2 = "REPLACE INTO minuteweather(wsid, idsite, timestamp, created, recno, baro, rf, temp, "
					."RH, solarkw, solarmj, windspeed, winddir, vmc, ec, soiltemp, gflux, soiltemptop, soiltempbot, "
					."lightning, batt, agg, cumrf,  local_ts) "
				."SELECT wsid, idsite, timestamp, created, recno, baro, rf, temp, "
					."RH, solarkw, solarmj, windspeed, winddir, vmc, ec, soiltemp, gflux, soiltemptop, soiltempbot, "
					."lightning, batt, agg, cumrf, local_ts "
				."FROM minutewx_import tmp "
					."LEFT JOIN minuteweather wx ON tmp.wsid = wx.wsid AND tmp.timestamp = wx.timestamp "
					."WHERE wx.timestamp IS NULL; ";
			}
			$sql = "REPLACE INTO {$tablename} SELECT * FROM {$tmp_tablename} WHERE idsite={$idsite} AND wsid={$wsid}; ";
			$db->exec($sql);
...
$force_load = TRUE;
foreach (glob("{$datapath}{$globprefix}*_WSoo.dat") as $filename) {
