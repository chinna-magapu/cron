...
function prepOutOfOrder_load($tablename, $ts, $idsite, $wsid) {
	...
	$sql = "DELETE FROM {$oo_tablename} WHERE idsite={$idsite} AND wsid={$wsid};";
	$db->exec($sql);
...
function process_file($dirpath, $fname){
	...
	if (($handle = fopen($fullfname, "r")) !== FALSE) {
		...
		if (empty($error)) {
			...
			if ($tablename == "minuteweatherfg") {
				if ($debug) echo "call AggregateToWeather with 1. {$idsite} 2. {$wsid} 3. {$first_ts} 4. {$last_ts} 5. {$oo_tablename} 6. {$dest_table} \n";
				$notes .= AggregateToWeather2rf($idsite, $wsid, $first_ts, $last_ts, $oo_tablename, $dest_table);
				...
				if ($debug) echo "call AggregateToWeather with 1. {$idsite} 2. {$wsid} 3. {$first_ts} 4. {$last_ts} 5. {$oo_tablename} 6. {$dest_table} \n";
				$notes .= AggregateToWeather($idsite, $wsid, $first_ts, $last_ts, $oo_tablename, $dest_table);
