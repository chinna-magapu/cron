...
function getUrlContent($url) {
	...
	$header = array('GET /1575051 HTTP/1.1',
		...
		'User-Agent: Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/71.0.3578.53 Safari/537.36',
	);
...
function loadHTML($url="",$fname="",$savefile=false) {
	...
	if ($dom->loadHTML($html)){
        if (true || $debug) echo "\nLoadHTML: Success";
		if ($savefile) $dom->saveHTMLFile("drf_wo.html");
		return $dom;
	}  else {
        if (true || $debug) echo "\nLoadHTML: FAILED";
        return false;
...
echo "DRF workouts scrape ".date(DATE_RFC822)."\n";
$dom = loadHTML("https://www1.drf.com/Results/Workouts.do","",true);
